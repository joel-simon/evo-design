...
def balanced_accuracy_score_np(true, pred):
    ...
    true_pos = np.logical_and(pred, true)
    true_neg = np.logical_and(np.logical_not(pred), np.logical_not(true))

    a = true_neg.sum() / n_true_neg if n_true_neg != 0 else 0
    b = true_pos.sum() / n_true_pos if n_true_pos != 0 else 0
